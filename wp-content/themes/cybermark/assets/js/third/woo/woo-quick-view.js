...
$j( document ).on( 'ready', function() {
	...
	if ( typeof cybermarkLocalize === 'undefined' ) {
		return false;
	}
	...
	$j( document ).on( 'click', '.owp-quick-view', function( e ) {
		...
		$j.ajax( {
			url: cybermarkLocalize.ajax_url,
			data: {
				action : 'cybermark_product_quick_view',
	...
	owpQVAddToCartHandler.prototype.onAddToCart = function( e ) {
		...
		if ( variation_id != '' ) {
			$j.ajax ({
				...
				type:'POST',
				data : {
			        action : 'cybermark_add_cart_single_product',
			        product_id : product_id,
			...
			$j.ajax ({
				...
				success:function(results) {
					...
					// Redirect to cart option
					if ( cybermarkLocalize.cart_redirect_after_add === 'yes' ) {
						window.location = cybermarkLocalize.cart_url;
						return;
					}
				}
			});
	...
	owpQVAddToCartHandler.prototype.updateButton = function( e, fragments, cart_hash, $button ) {
		...
		if ( $button ) {
			...
			// View cart text.
			if ( ! cybermarkLocalize.is_cart && $button.parent().find( '.added_to_cart' ).length === 0 ) {
				$button.after( ' <a href="' + cybermarkLocalize.cart_url + '" class="added_to_cart wc-forward" title="' +
					cybermarkLocalize.view_cart + '">' + cybermarkLocalize.view_cart + '</a>' );
			}
		}
	};
