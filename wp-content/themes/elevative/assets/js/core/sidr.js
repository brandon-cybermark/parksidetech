...
MOBILE SCRIPT
============================================== */
function elevativeMobileMenu( event ) {
	"use strict"

	if ( typeof elevativeLocalize.sidrSource !== 'undefined'
		&& $j( 'body' ).hasClass( 'sidebar-mobile' ) ) {

		// Add sidr
		$j( '.mobile-menu' ).sidr( {
			name     : 'sidr',
			source   : elevativeLocalize.sidrSource,
			side     : elevativeLocalize.sidrSide,
			displace : elevativeLocalize.sidrDisplace,
			speed    : 300,
			renaming : true,
			bind     : 'click',
			...
			onOpen   : function onOpen() {
				...
				// Check localization
				if ( elevativeLocalize.sidrDropdownTarget == 'link' ) {
				...
				$j( 'body' ).append( '<div class="elevative-sidr-overlay"></div>' );
				$j( '.elevative-sidr-overlay' ).fadeIn( 300 );

				// Close sidr when clicking overlay
				$j( '.elevative-sidr-overlay' ).on( 'click', function() {
					$j.sidr( 'close', 'sidr' );
					return false;
				} );
			...
			onClose : function onClose() {
				...
				// FadeOut overlay
				$j( '.elevative-sidr-overlay' ).fadeOut( 300, function() {
					$j( this ).remove();
				} );
			}
