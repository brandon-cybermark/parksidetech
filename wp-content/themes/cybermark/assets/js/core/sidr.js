...
MOBILE SCRIPT
============================================== */
function cybermarkMobileMenu( event ) {
	"use strict"

	if ( typeof cybermarkLocalize.sidrSource !== 'undefined'
		&& $j( 'body' ).hasClass( 'sidebar-mobile' ) ) {

		// Add sidr
		$j( '.mobile-menu' ).sidr( {
			name     : 'sidr',
			source   : cybermarkLocalize.sidrSource,
			side     : cybermarkLocalize.sidrSide,
			displace : cybermarkLocalize.sidrDisplace,
			speed    : 300,
			renaming : true,
			bind     : 'click',
			...
			onOpen   : function onOpen() {
				...
				// Check localization
				if ( cybermarkLocalize.sidrDropdownTarget == 'link' ) {
				...
				$j( 'body' ).append( '<div class="cybermark-sidr-overlay"></div>' );
				$j( '.cybermark-sidr-overlay' ).fadeIn( 300 );

				// Close sidr when clicking overlay
				$j( '.cybermark-sidr-overlay' ).on( 'click', function() {
					$j.sidr( 'close', 'sidr' );
					return false;
				} );
			...
			onClose : function onClose() {
				...
				// FadeOut overlay
				$j( '.cybermark-sidr-overlay' ).fadeOut( 300, function() {
					$j( this ).remove();
				} );
			}
