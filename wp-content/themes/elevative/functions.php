<?php
/**
 * Elevative functions and definitions
 *
 ...
 */
/**
 * Elevative only works in WordPress 4.7 or later.
 */
 ...
 */
function elevative_setup() {
	/*
	 * Make theme available for translation.
	 * Translations can be filed at WordPress.org. See: https://translate.wordpress.org/projects/wp-themes/elevative
	 * If you're building a theme based on Elevative, use a find and replace
	 * to change 'elevative' to the name of your theme in all the template files.
	 */
	load_theme_textdomain( 'elevative' );
	...
	add_image_size( 'elevative-thumbnail-avatar', 100, 100, true );
	// Set the default content width.
	$GLOBALS['content_width'] = 1170;
	// This theme uses wp_nav_menu() in two locations.
	register_nav_menus(
		array(
			'top'    => __( 'Top Menu', 'elevative' ),
			'main-nav' => __( 'Main Menu', 'elevative' ),
			'footer' => __( 'Footer Menu', 'elevative' ),
		)
	);
	/*
...
add_action( 'after_setup_theme', 'elevative_setup' );
/**
 * Register custom fonts.
 */
function elevative_fonts_url() {
	$fonts_url = '';
	/*
	 ...
	 */
	$libre_franklin = _x( 'on', 'Libre Franklin font: on or off', 'elevative' );
 ...
 */
function elevative_widgets_init() {
	register_sidebar(
		array(
			'name'          => __( 'Blog Sidebar', 'elevative' ),
			'id'            => 'sidebar-1',
			'description'   => __( 'Add widgets here to appear in your sidebar on blog posts and archive pages.', 'elevative' ),
...
add_action( 'widgets_init', 'elevative_widgets_init' );
/**
 ...
 * @since Elevative 1.0
 */
function elevative_javascript_detection() {
	echo "<script>(function(html){html.className = html.className.replace(/\bno-js\b/,'js')})(document.documentElement);</script>\n";
}
add_action( 'wp_head', 'elevative_javascript_detection', 0 );
/**
 * Enqueues scripts and styles.
 */
function elevative_scripts() {
	...
	wp_enqueue_script( 'elevative-skip-link-focus-fix', get_theme_file_uri( '/assets/js/skip-link-focus-fix.js' ), array(), '1.0', true );
	$elevative_l10n = array(
		'quote' => elevative_get_svg( array( 'icon' => 'quote-right' ) ),
	);
	if ( has_nav_menu( 'top' ) ) {
		//wp_enqueue_script( 'elevative-navigation', get_theme_file_uri( '/assets/js/navigation.js' ), array( 'jquery' ), '1.0', true );
		$elevative_l10n['expand']   = __( 'Expand child menu', 'elevative' );
		$elevative_l10n['collapse'] = __( 'Collapse child menu', 'elevative' );
		$elevative_l10n['icon']     = elevative_get_svg(
			array(
				'icon'     => 'angle-down',
				'fallback' => true,
			)
	...
	wp_localize_script( 'elevative-skip-link-focus-fix', 'elevativeScreenReaderText', $elevative_l10n );
	if ( is_singular() && comments_open() && get_option( 'thread_comments' ) ) {
		wp_enqueue_script( 'comment-reply' );
	}
}
add_action( 'wp_enqueue_scripts', 'elevative_scripts' );
/**
 ...
 */
function elevative_content_image_sizes_attr( $sizes, $size ) {
	$width = $size[0];
	if ( 740 <= $width ) {
...
add_filter( 'wp_calculate_image_sizes', 'elevative_content_image_sizes_attr', 10, 2 );
/**
 ...
 */
function elevative_header_image_tag( $html, $header, $attr ) {
	if ( isset( $attr['sizes'] ) ) {
		$html = str_replace( $attr['sizes'], '100vw', $html );
	}
	return $html;
}
add_filter( 'get_header_image_tag', 'elevative_header_image_tag', 10, 3 );
/**
 * Add custom image sizes attribute to enhance responsive image functionality
 * for post thumbnails.
 *
 ...
 */
function elevative_post_thumbnail_sizes_attr( $attr, $attachment, $size ) {
...
add_filter( 'wp_get_attachment_image_attributes', 'elevative_post_thumbnail_sizes_attr', 10, 3 );
/**
...
//Elevative Landing Page custom post
function cpts_landing_page() {
    $labels = array(
        "name" => __( "Landing Page", "elevative" ),
        "singular_name" => __( "Landing Page", "elevative" ),
        "menu_name" => __( "Landing Pages", "elevative" ),
        "all_items" => __( "All Landing Pages", "elevative" ),
        "add_new" => __( "Add Landing Page", "elevative" ),
        "add_new_item" => __( "Add New Landing Page", "elevative" ),
        "edit_item" => __( "Edit Landing Page", "elevative" ),
        "new_item" => __( "New Landing Page", "elevative" ),
        "view_item" => __( "View Landing Page", "elevative" ),
        "view_items" => __( "View Landing Pages", "elevative" ),
        "search_items" => __( "Search Landing Pages", "elevative" ),
        "not_found" => __( "No Landing Pages Found", "elevative" ),
    );
    $args = array(
        "label" => __( "Landing Page", "elevative" ),
        "labels" => $labels,
